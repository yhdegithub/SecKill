...
#解决思路 ：
 ...
 ## 1.   缓存优化： Redis 。   
    ...
    2）修改 ： 现在redis中读状态 。加分布式锁，删除缓存 ，写入DB ， 回填缓存，解除锁 ， 结束。
      在本项目中，我们可以利用此来限流 。当redis中存储的商品余额为<=0的时候 ，我们可以直接返回抢购失败 ； 否则 ，加上分布式锁失败的不断尝试 ；成功的 ，返回抢购成功，请等待，对商品的库品修改-1及下订单写入做事务处理 ，并使用cannal的框架写回到redis
